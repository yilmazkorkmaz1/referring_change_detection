    ...
    def __call__(
        ...
        **kwargs,
        ...
        if not output_type == "latent":
            image1 = self.vae.decode(latents[:,:4] / self.vae.config.scaling_factor, return_dict=False)[0]
            image2 = F.interpolate(latents[:,4:], size=(512, 512), mode='bilinear', align_corners=False)
            # Binarize mask at threshold 0 (before conversion to PIL)
            image2 = torch.where(image2 >= 0.0, torch.ones_like(image2), -torch.ones_like(image2))
        ...
        image1 = self.image_processor.postprocess(image1, output_type=output_type, do_denormalize=do_denormalize)
