...
def print_iou(iou, Score, Sek, Semantic_IoU, recall, precision, freq_IoU, mean_pixel_acc, pixel_acc, class_names=None, show_no_back=False, no_print=False):
    n = iou.size
    lines = []
    for i in range(n):
    ...
    lines.append('----------     %-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%' % 
                ('mean_IoU', mean_IoU * 100, 'precision', precision*100, 'recall', recall*100, 'freq_IoU', freq_IoU*100, 
                    'mean_pixel_acc', mean_pixel_acc*100, 'pixel_acc', pixel_acc*100, 'SeK', Sek*100, 'Score', Score*100, 
                    'Semantic_IoU', Semantic_IoU*100))
    line = "\n".join(lines)
    if not no_print:
        print(line)
    return line
