...
class MambaDecoder(nn.Module):
    def __init__(self, 
        ...
        for i_layer in range(self.num_layers):
            if i_layer == 0:
                ...
                attention_layer = attention.SpatialTransformer((embed_dim * 2 ** (self.num_layers - 1 - i_layer)), 8, 64, 1, 0.1, context_dim=512, rearrange_input=True)
            else:
    ...
    def forward_up_features(self, inputs, contexts):
        if not self.deep_supervision:
            for inx, (layer_up, attention_layer) in enumerate(zip(self.layers_up, self.attention_blocks)):
                if inx == 0:
                    x = inputs[3 - inx]  
                    x = x.permute(0, 2, 3, 1).contiguous()  
                    x = attention_layer(x, context=contexts)
                    y = layer_up(x)  
                else:
                    B, C, H, W = inputs[3 - inx].shape
                    y = attention_layer(y, context=contexts)
    ...
    def forward(self, inputs, contexts):
        if not self.deep_supervision:
            x = self.forward_up_features(inputs, contexts) # B, H, W, C
