...
class RGBXTransformer(nn.Module):
    ...
    def forward_features(self, x_A, x_B, captions):
        """
        x_A: B x C x H x W
        """
        B = x_A.shape[0]
    ...
    def forward(self, x_A, x_B, captions):
        out = self.forward_features(x_A, x_B, captions)
        return out

class vssm_tiny(RGBXTransformer):
    def __init__(self, use_imagenet_pretrain=True, **kwargs):
        super(vssm_tiny, self).__init__(
            depths=[2, 2, 9, 2], 
            dims=96,
            pretrained='pretrained/vmamba/vssmtiny_dp01_ckpt_epoch_292.pth' if use_imagenet_pretrain else None,
            mlp_ratio=0.0,
            downsample_version='v1',
            drop_path_rate=0.2,
        )
...
class vssm_small(RGBXTransformer):
    def __init__(self, use_imagenet_pretrain=True, **kwargs):
        super(vssm_small, self).__init__(
            depths=[2, 2, 27, 2],
            dims=96,
            pretrained='pretrained/vmamba/vssmsmall_dp03_ckpt_epoch_238.pth' if use_imagenet_pretrain else None,
            mlp_ratio=0.0,
            downsample_version='v1',
            drop_path_rate=0.3,
        )
        # avoid printing on every distributed rank
...
class vssm_base(RGBXTransformer):
    def __init__(self, use_imagenet_pretrain=True, **kwargs):
        super(vssm_base, self).__init__(
            depths=[2, 2, 27, 2],
            dims=128,
            pretrained='pretrained/vmamba/vssmbase_dp06_ckpt_epoch_241.pth' if use_imagenet_pretrain else None,
            mlp_ratio=0.0,
            downsample_version='v1',
