...
class DualSwinTransformer(nn.Module):
    ...
    def forward(self, x, x_d, captions=None):
        """Forward function."""
...
class swin_b(DualSwinTransformer):
    def __init__(self, **kwargs):
        super(swin_b, self).__init__(pretrain_img_size=224, patch_size=4, in_chans=3, embed_dim=128, depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32], 
                                     window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.3, drop_path_rate=0.1,
                                     norm_layer=nn.LayerNorm, ape=False, patch_norm=True, out_indices=(0, 1, 2, 3), frozen_stages=-1, use_checkpoint=False)

class swin_l(DualSwinTransformer):
    def __init__(self, **kwargs):
        super(swin_l, self).__init__(pretrain_img_size=384, patch_size=4, in_chans=3, embed_dim=192, depths=[2, 2, 18, 2], num_heads=[6, 12, 24, 48], 
                                     window_size=12, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.3, drop_path_rate=0.2,
                                     norm_layer=nn.LayerNorm, ape=False, patch_norm=True, out_indices=(0, 1, 2, 3), frozen_stages=-1, use_checkpoint=False)
