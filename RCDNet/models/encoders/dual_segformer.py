...
class RGBXTransformer(nn.Module):
    ...
    def forward(self, x_rgb, x_e, captions=None):
...
def load_dualpath_model(model, model_file):
    ...
    # remove the weight of attn
    state_dict_wo_attn = {}
    for k, v in state_dict.items():
        if k.find('attn') < 0:
            state_dict_wo_attn[k] = v
    t_ioend = time.time()

    model_state_dict = model.state_dict()
    updated_state_dict = {k: v for k, v in state_dict_wo_attn.items() if k in model_state_dict and v.size() == model_state_dict[k].size()}
    model_state_dict.update(updated_state_dict)
    model.load_state_dict(model_state_dict)

  #  model.load_state_dict(state_dict_wo_attn, strict=False)
    del state_dict
    del state_dict_wo_attn
...
class mit_b2(RGBXTransformer):
    def __init__(self, fuse_cfg=None, **kwargs):
        super(mit_b2, self).__init__(
            patch_size=4, embed_dims=[64, 128, 256, 512], num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4],
