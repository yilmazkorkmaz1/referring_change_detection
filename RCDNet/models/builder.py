...
class EncoderDecoder(nn.Module):
    def __init__(self, cfg=None, criterion=nn.CrossEntropyLoss(reduction='mean', ignore_index=255), criterion2 = None, norm_layer=nn.BatchNorm2d):
        ...
        if cfg.backbone == 'swin_s':
            ...
            logger.info('Using backbone: Swin-Transformer-Large')

            from .encoders.dual_swin import swin_l as backbone
            ...
            self.backbone = backbone(use_imagenet_pretrain=cfg.use_imagenet_pretrain)
    ...
    def encode_decode(self, rgb, modal_x, captions):
        """Encode images with backbone and decode into a semantic segmentation
        map of the same size as input."""
        if not self.deep_supervision:
            orisize = rgb.shape
            x = self.backbone(rgb, modal_x, captions)
            out = self.decode_head.forward(x, captions)
            ...
            x = self.backbone(rgb, modal_x, captions)
            x_last, x_output_0, x_output_1, x_output_2 = self.decode_head.forward(x, captions)
    ...
    def forward(self, rgb, modal_x, label=None, captions=None):
        if not self.deep_supervision:
            if self.aux_head:
                ...
                out = self.encode_decode(rgb, modal_x, captions)
            if label is not None:
                # if len(label.shape) < 4:
                #     loss = self.criterion(out, label[:,None,:,:].long())
                # else:
    
                loss = self.criterion(out, label.float())
                if self.criterion2 is not None:
                    loss += self.criterion2(out, label.long())
                if self.aux_head:
